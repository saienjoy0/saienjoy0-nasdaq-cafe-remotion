...
const progressAt = (content: PublicMainContent, revealAtMs: number, durationSeconds = 0.62) => spring({
  fps: FPS,
...
const AnimatedNumber: React.FC<{content: PublicMainContent; number: PublicNumber; size?: number}> = ({content, number, size = 64}) => {
  const progress = progressAt(content, number.revealAtMs);
...
const OpeningContradiction: React.FC<{content: PublicMainContent}> = ({content}) => {
  ...
  return <Surface accent={color.emphasis} style={{padding: "34px 44px", display: "grid", gridTemplateRows: "auto 1fr auto", gap: 22}}>
    <div style={{...enter(content, leadAt), display: "flex", alignItems: "center", justifyContent: "space-between"}}><div><Tag tone="positive">昨夜の方向</Tag><div style={{marginTop: 10, fontSize: 34, color: color.muted, fontWeight: 900}}>NASDAQ</div></div>{lead ? <div style={{color: toneColor(lead.tone)}}><AnimatedNumber content={content} number={lead} size={88}/></div> : <div style={{fontSize: 72, color: color.positive, fontWeight: 950}}>{content.cards[0]?.lines[0]?.value ?? content.headline}</div>}</div>
    <div style={{display: "grid", gridTemplateColumns: `repeat(${Math.max(1, Math.min(3, facts.length || content.texts.length))},minmax(0,1fr))`, gap: 18}}>{facts.length > 0 ? facts.map((item) => <div key={item.key} style={{...enter(content, item.revealAtMs), borderRadius: 22, padding: "24px 20px", background: `${toneColor(item.tone)}10`, border: `3px solid ${toneColor(item.tone)}`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center"}}><div style={{fontSize: 30, fontWeight: 950}}>{item.label}</div><div style={{marginTop: 13, color: toneColor(item.tone)}}><AnimatedNumber content={content} number={item} size={52}/></div></div>) : content.texts.slice(0, 3).map((text, index) => <div key={text} style={{...enter(content, content.beatStartMs + (index + 1) * 650), borderRadius: 22, padding: "24px 20px", background: "rgba(82,118,145,.10)", border: `3px solid ${index === 0 ? color.positive : index === 1 ? color.warning : color.negative}`, display: "flex", alignItems: "center", justifyContent: "center", textAlign: "center", fontSize: 34, fontWeight: 950}}>{text}</div>)}</div>
    <div style={{...enter(content, content.beatStartMs + Math.min(2800, (content.beatEndMs - content.beatStartMs) * 0.58)), display: "flex", justifyContent: "space-between", gap: 24}}><div style={{fontSize: 30, color: color.muted, fontWeight: 850}}>全部が同じ理由で動いた夜ではない</div><div style={{fontSize: 38, color: color.emphasis, fontWeight: 950}}>{content.screenQuestion}</div></div>
  </Surface>;
};
...
const BulletComparison: React.FC<{content: PublicMainContent}> = ({content}) => {
  ...
  const progress = progressAt(content, actual.revealAtMs);
  return <Surface accent={color.positive} style={{padding: "42px 50px", display: "grid", gridTemplateRows: "auto 1fr auto", gap: 30}}><div style={{...enter(content, actual.revealAtMs), display: "flex", justifyContent: "space-between", alignItems: "center"}}><div><Tag>予想と実績</Tag><div style={{marginTop: 13, fontSize: 36, fontWeight: 950}}>{actual.label}</div></div><div style={{color: toneColor(actual.tone)}}><AnimatedNumber content={content} number={actual} size={73}/></div></div><div style={{...enter(content, actual.revealAtMs + 250), position: "relative", alignSelf: "center", height: 150, borderRadius: 24, background: "rgba(82,118,145,.14)", border: "2px solid rgba(82,118,145,.30)", overflow: "hidden"}}><div style={{position: "absolute", left: 0, top: 0, bottom: 0, width: `${clamp((actualValue / max) * progress) * 100}%`, background: "linear-gradient(90deg,rgba(7,134,95,.55),rgba(7,134,95,.96))"}}/>{expectedValue > 0 ? <div style={{position: "absolute", left: `${clamp(expectedValue / max) * 100}%`, top: 0, bottom: 0, width: 7, background: color.warning, boxShadow: "0 0 0 4px rgba(255,255,255,.82)"}}/> : null}<div style={{position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 31px", fontSize: 31, fontWeight: 950}}><span>実績</span><span>{actual.comparison ?? "市場予想"}</span></div></div>{gap ? <div style={{...enter(content, gap.revealAtMs), display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px 28px", borderRadius: 21, background: "rgba(112,70,168,.10)", border: `3px solid ${color.emphasis}`}}><div style={{fontSize: 30, color: color.muted, fontWeight: 900}}>{gap.label}</div><div style={{color: color.emphasis}}><AnimatedNumber content={content} number={gap} size={58}/></div></div> : null}</Surface>;
};
...
const TailwindHeadwind: React.FC<{content: PublicMainContent}> = ({content}) => {
  ...
  const left = texts.filter((_, index) => index % 2 === 0); const right = texts.filter((_, index) => index % 2 === 1);
  const lane = (items: string[], label: string, tone: Tone, offset: number) => <div style={{display: "grid", gridTemplateRows: "auto 1fr", gap: 16}}><div style={{textAlign: "center"}}><Tag tone={tone}>{label}</Tag></div><div style={{display: "grid", gap: 14, alignContent: "center"}}>{items.map((text, index) => <div key={text} style={{...enter(content, content.beatStartMs + (offset + index) * 620, "x"), padding: "19px 22px", borderRadius: 19, background: `${toneColor(tone)}0f`, border: `3px solid ${toneColor(tone)}`, textAlign: "center", fontSize: 30, lineHeight: 1.23, fontWeight: 930}}>{text}</div>)}</div></div>;
...
const VerificationMatrix: React.FC<{content: PublicMainContent}> = ({content}) => {
  ...
  const items = content.cards.length > 0 ? content.cards.map((card) => card.lines[0]?.value ?? card.title) : content.texts; const midpoint = Math.ceil(items.length / 2); const lanes = [items.slice(0, midpoint), items.slice(midpoint)];
  return <Surface accent={color.warning} style={{padding: "28px 34px", display: "grid", gridTemplateRows: "auto 1fr auto", gap: 18}}><div style={{display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20}}><div style={{textAlign: "center"}}><Tag tone="positive">{labels[0]}</Tag></div><div style={{textAlign: "center"}}><Tag tone="warning">{labels[1]}</Tag></div></div><div style={{display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20}}>{lanes.map((laneItems, laneIndex) => <div key={labels[laneIndex]} style={{display: "grid", gap: 15, alignContent: "center"}}>{laneItems.map((text, index) => <div key={text} style={{...enter(content, content.beatStartMs + (laneIndex * midpoint + index) * 620), display: "grid", gridTemplateColumns: "44px 1fr", gap: 14, alignItems: "center", padding: "18px 20px", borderRadius: 18, background: laneIndex === 0 ? "rgba(7,134,95,.09)" : "rgba(186,107,0,.09)", border: `2px solid ${laneIndex === 0 ? "rgba(7,134,95,.30)" : "rgba(186,107,0,.30)"}`, fontSize: 29, lineHeight: 1.23, fontWeight: 900}}><div style={{width: 34, height: 34, borderRadius: 99, display: "flex", alignItems: "center", justifyContent: "center", color: color.white, background: laneIndex === 0 ? color.positive : color.warning, fontSize: 23, fontWeight: 950}}>{laneIndex === 0 ? "+" : "−"}</div>{text}</div>)}</div>)}</div><div style={{textAlign: "center", color: color.emphasis, fontSize: 33, fontWeight: 950}}>{content.primaryElement}</div></Surface>;
};
...
const FinalAssembly: React.FC<{content: PublicMainContent}> = ({content}) => {
  ...
  return <Surface accent={color.emphasis} style={{padding: "40px 52px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center"}}><div style={enter(content, content.beatStartMs)}><Tag tone="emphasis">今朝の結論</Tag></div><div style={{display: "flex", gap: 15, marginTop: 26, width: "100%", justifyContent: "center"}}>{chips.map((chip, index) => <div key={`${index}-${chip}`} style={{...enter(content, content.beatStartMs + (index + 1) * 520), flex: 1, maxWidth: 315, padding: "15px 17px", borderRadius: 17, background: "rgba(82,118,145,.10)", border: "2px solid rgba(82,118,145,.30)", fontSize: 26, lineHeight: 1.22, fontWeight: 900}}>{chip}</div>)}</div><div style={{...enter(content, content.beatStartMs + Math.min(2600, (content.beatEndMs - content.beatStartMs) * 0.58)), marginTop: 32, fontSize: 70, lineHeight: 1.14, color: color.emphasis, fontWeight: 950}}>{content.primaryElement || content.headline}</div><div style={{marginTop: 30, width: `${62 + content.holdProgress * 18}%`, height: 7, borderRadius: 99, background: `linear-gradient(90deg,transparent,${color.cyan},transparent)`}}/></Surface>;
};
