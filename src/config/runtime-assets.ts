...
export type RuntimeAssetContext = {
  ...
  manifest: typeof productionAssetManifest;
...
export const loadRuntimeAssetContext = async (
  ...
  if (typeof registry.episodeDate !== "string" || !/^\d{4}-\d{2}-\d{2}$/.test(registry.episodeDate)) {
  ...
  const assets: Record<string, {type: string; role?: string; path: string}> = {
  ...
  for (const [index, item] of registry.assets.entries()) {
    ...
    const staticAsset = productionAssetManifest.assets[entry.assetId as keyof typeof productionAssetManifest.assets] as
      | {path: string; type?: string; role?: string}
      | undefined;
    if (staticAsset) {
      if (entry.source !== "static" || entry.path !== staticAsset.path) {
        throw new Error(`runtime asset registry attempted to override static assetId: ${entry.assetId}`);
      }
      continue;
    }
    ...
    assets[entry.assetId] = {type: "image", role: "main-media", path: entry.path};
  ...
  return {
    ...
    manifest: {...productionAssetManifest, assets},
    paths,
  };
};
