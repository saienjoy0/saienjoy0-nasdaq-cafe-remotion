#!/usr/bin/env python3
"""Make optional templateVariant mirror the authoritative templateConfig.variant."""
...
def main() -> int:
    ...
    for scene in spec["scenes"]:
        for beat in scene["visualBeats"]:
            authoritative = beat["templateConfig"]["variant"]
            previous = beat.get("templateVariant")
            beat["templateVariant"] = authoritative
    ...
    normalization["templateVariantReboundCount"] = len(changes)
    ...
    report["templateVariantRebindings"] = changes
    args.report.write_text(
        json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    print(json.dumps({"status": "rebound", "count": len(changes), "sha256": sha}))
    return 0
