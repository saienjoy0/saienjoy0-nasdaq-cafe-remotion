...
const authoredScopes = spec.scenes.flatMap((scene) =>
  scene.visualBeats.map((beat) => beat.semanticScope),
);
assert.deepEqual(
  candidateInput.beats.map((beat) => beat.semanticScope),
  authoredScopes,
  "VisualCandidateInput must preserve each authored semanticScope",
);
...
for (const candidate of catalog.candidates) {
  const authored = spec.scenes
    .flatMap((scene) => scene.visualBeats)
    .find((beat) => beat.beatId === candidate.visualBeatId);
  assert.ok(authored);
...
assert.deepEqual(
  compiled.spec.scenes.flatMap((scene) => scene.visualBeats.map((beat) => beat.semanticScope)),
  authoredScopes,
  "Visual Director compile must preserve semanticScope",
);
...
const tamperedPlan = {
  ...plan,
  candidateCatalogSha256: sha256Json(tampered),
};
assert.throws(
  ...
  /semanticScope|semantic scope/i,
  "candidate semanticScope drift must fail closed",
);

const legacy = makeCurrentVisualDirectorFixture();
const legacyCatalog = buildVisualCandidateCatalog({
  spec: legacy,
  sourceRenderSpecSha256: sha256Json(legacy),
});
